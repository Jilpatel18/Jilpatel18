<div align="center">

<img src="https://capsule-render.vercel.app/api?type=venom&height=200&color=gradient&customColorList=0,2,2,5,30&text=Jilpatel18&fontSize=70&fontColor=00FF41&fontAlignY=55&desc=Cybersecurity%20Student%20%7C%20CTF%20Player%20%7C%20Ethical%20Hacker&descSize=16&descAlignY=75&descColor=ffffff&animation=twinkling&stroke=00FF41&strokeWidth=1" width="100%"/>

</div>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=00FF41&center=true&vCenter=true&multiline=false&width=600&lines=Cybersecurity+Student+%F0%9F%9B%A1%EF%B8%8F;CTF+Player+%E2%9A%94%EF%B8%8F;Ethical+Hacker+in+Training+%F0%9F%94%90;Breaking+things+to+fix+them+%F0%9F%94%93;300%2B+LeetCode+Problems+Solved+%F0%9F%94%A5)](https://git.io/typing-svg)

</div>

<p align="right">
  <img src="https://visitor-badge.laobi.icu/badge?page_id=Jilpatel18.Jilpatel18&color=0D1117&labelColor=00FF41" alt="visitors" />
</p>

<br/>

```bash
┌──(jilu㉿kali)-[~/profile]
└─$ whoami
  > Cybersecurity Student | Parul University, Sem 4
  > CTF enthusiast | Web Security | Network Recon
  > Building: Django projects + Secure Web Apps
  > Currently learning: Pentesting, OSCP prep
  > Fun fact: I solve LeetCode problems AND crack CTF flags
```

<br/>

<h2 align="center">🛠️ Technical Arsenal</h2>
<p align="center">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=py,c,js,bash,mysql,django,react,flask,linux,git,github,docker,vscode&theme=dark" />
  </a>
</p>
<p align="center">
  <img src="https://img.shields.io/badge/Kali_Linux-0D1117?style=for-the-badge&logo=kali-linux&logoColor=00FF41" />
  <img src="https://img.shields.io/badge/Burp_Suite-0D1117?style=for-the-badge&logo=burpsuite&logoColor=00FF41" />
  <img src="https://img.shields.io/badge/Nmap-0D1117?style=for-the-badge&logo=nmap&logoColor=00FF41" />
  <img src="https://img.shields.io/badge/Wireshark-0D1117?style=for-the-badge&logo=wireshark&logoColor=00FF41" />
  <img src="https://img.shields.io/badge/Metasploit-0D1117?style=for-the-badge&logo=metasploit&logoColor=00FF41" />
</p>

<br/>

<h2 align="center">📈 GitHub Contributions</h2>
<div align="center">
  <img src="https://raw.githubusercontent.com/Jilpatel18/Jilpatel18/output/github-snake-dark.svg" alt="contribution snake"/>
</div>

<br/>

<h2 align="center">📊 GitHub Stats</h2>
<div align="center">
<div align="center">
<table>
  <tr>
    <td>
      <img src="https://github-stats-alpha.vercel.app/api?username=Jilpatel18&cc=0D1117&tc=00FF41&ic=ffffff&bc=0D1117" />
    </td>
    <td>
      <img src="https://streak-stats.demolab.com?user=Jilpatel18&theme=dark&hide_border=true&ring=00FF41&fire=FF6B35&currStreakLabel=00FF41&background=0D1117" />
    </td>
  </tr>
</table>

<img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Jilpatel18&theme=github_dark" />
<img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Jilpatel18&theme=github_dark" />

</div>
<br/>
<div align="center">
  <!-- <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Jilpatel18&layout=compact&theme=github_dark&hide_border=true&title_color=00FF41&text_color=ffffff&bg_color=0D1117&langs_count=8" />
</div> -->



<h2 align="center">💻 Current Projects</h2>
<div align="center">

| Project | Stack | Status |
|---------|-------|--------|
| 🛡️ **Kavaach** — Parametric Income Insurance | React + Django + GSAP | 🟢 Active |
| 🎮 **Chess Multiplayer** | Django Channels + Redis | 🟡 Building |
| 🏎️ **F1 Dashboard** | Vanilla JS + Jolpica API | ✅ Done |
| 🎌 **ANIZED Anime Platform** | Django + GSAP + Vanta.js | 🟡 Building |

</div>

<br/>

<h2 align="center">🎯 Learning Radar & CTF Focus</h2>

```text
+-------------------------------------------------------------+
|  🔍 Web Application Pentesting (OWASP Top 10)               |
|  🌐 Network Recon & Scanning                                |
|  💣 CTF: Crypto, Forensics, Reverse Engineering             |
|  📖 Studying: TryHackMe / HackTheBox / PicoCTF              |
|  📜 Goal: OSCP Certification                                |
+-------------------------------------------------------------+
```

<br/>

<h2 align="center">🌐 Connect with Me</h2>
<p align="center">
  <a href="https://www.linkedin.com/in/jilvaghasiya18/"><img src="https://img.shields.io/badge/LinkedIn-0D1117?style=for-the-badge&logo=linkedin&logoColor=00FF41" alt="LinkedIn" /></a>
  <a href="https://x.com/sudo_apt_jil_18"><img src="https://img.shields.io/badge/X-0D1117?style=for-the-badge&logo=x&logoColor=00FF41" alt="Twitter" /></a>
  <a href="https://leetcode.com/u/MrRobot_23/"><img src="https://img.shields.io/badge/LeetCode-0D1117?style=for-the-badge&logo=leetcode&logoColor=00FF41" alt="LeetCode" /></a>
</p>
<br/>
<p align="center">
  <i>Made with ❤️ and too many CTF sleepless nights</i>
</p>
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=0,0D1117,50,00FF41,100,0D1117&height=100&section=footer" width="100%"/>
</p>
